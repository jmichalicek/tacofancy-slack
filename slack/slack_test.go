...
func TestSlashCommandBuildResponseForTacoDefault(t *testing.T) {
    slashCommand := SlashCommand{
		Command: "/taco", Text: "",
        Token: "testtoken", ResponseURL: "https://example.org/",
        ChannelId: "fake", UserId: "fake"}


    var apiStub = httptest.NewServer(
        http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {w.WriteHeader(http.StatusOK)}))
    // URL not updated by doing this...  need to find correct way to do it
    // Possibly do the SlashCommandResponder interface and then I can pass
    // that into BuildResponse() and it can have tacofancy api stuff
    // or just make tacofancy api client and have a tacofancy client
    // attached to SlashCommand for now... which is crappy but would work
    // since this is small
    tacofancy.BaseUrl = apiStub.URL
    ...
    if !reflect.DeepEqual(response, expectedResponse) {
        t.Errorf("Expected SlashCommandResponse of \n\n", expectedResponse, "\n\n but got \n\n", response)
    }
}
